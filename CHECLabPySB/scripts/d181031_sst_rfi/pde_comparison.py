...
def main():
    x_checs, y_checs = np.loadtxt(get_data("sst_rfi/pde/CHECS.dat"), unpack=True)
    x_prod3, y_prod3 = np.loadtxt(get_data("sst_rfi/pde/Prod3.dat"), unpack=True)
    x_prod4, y_prod4 = np.loadtxt(get_data("sst_rfi/pde/Prod4.dat"), unpack=True)
    ...
    p_pde.save(get_plot("sst_rfi/pde.pdf"))
